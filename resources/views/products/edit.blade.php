...
<div class="max-w-3xl mx-auto">
  <div class="bg-white shadow rounded-lg p-6">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">
      <i class="fa-solid fa-pen mr-2"></i> Edit Produk
    </h1>
    <form action="{{ route('products.update', $product) }}" method="POST">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input type="text" name="name" id="name" class="w-full p-3 border rounded focus:outline-none focus:ring focus:border-blue-300" value="{{ old('name', $product->name) }}" required>
        </div>
        <!-- Stok -->
        <div>
          <label for="stock" class="block text-gray-700 font-semibold mb-1">Stok</label>
          <input type="number" name="stock" id="stock" class="w-full p-3 border rounded focus:outline-none focus:ring focus:border-blue-300" value="{{ old('stock', $product->stock) }}" required>
        </div>
        ...
        <div>
          ...
          <input type="number" step="0.01" name="price" id="price" class="w-full p-3 border rounded focus:outline-none focus:ring focus:border-blue-300" value="{{ old('price', $product->price) }}" required>
        </div>
      </div>
      <!-- Tombol Submit -->
      <div class="mt-6">
